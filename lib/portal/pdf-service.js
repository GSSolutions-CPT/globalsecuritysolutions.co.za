...
const generatePDF = async (docType, data, settings = {}) => {
    ...
    try {
        ...
        if (docType !== 'Purchase Order') {
            ...
            const hasDeposit = data.payment_type === 'deposit'
            const depPercent = data.deposit_percentage || 75
            const depRatio = (hasDeposit ? depPercent : 100) / 100
            const firstPaymentVal = totalVal * depRatio
            const secondPaymentVal = totalVal * (1 - depRatio)

            let payY = bankY + 4
            text("PAYMENT HISTORY", margin, payY, { size: 8, style: 'bold', color: COLORS.TEXT_MAIN })
            doc.setDrawColor(...COLORS.BORDER)
            doc.setLineWidth(0.2)
            doc.line(margin, payY + 2, margin + 60, payY + 2)
            payY += 7

            const firstPayLabel = hasDeposit ? `${depPercent}% Deposit` : "100% Upfront"
            const firstPayStatus = data.admin_approved || data.status === 'Paid' || data.status === 'Approved' || data.status === 'Accepted' || docType === 'Invoice' ? "Approved" : "Pending"
            text(`${firstPayLabel}:`, margin, payY, { size: 8, color: COLORS.TEXT_MUTED })
            text(`R ${firstPaymentVal.toFixed(2)} (${firstPayStatus})`, margin + 25, payY, { size: 8, color: firstPayStatus === 'Approved' ? [16, 124, 65] : [220, 100, 0], style: 'bold' })
            payY += 5

            if (hasDeposit && secondPaymentVal > 0) {
                const secondPayStatus = data.final_payment_approved || data.status === 'Paid' ? "Approved" : (data.final_payment_proof ? "Pending Review" : "Unpaid")
                text(`${100 - depPercent}% Balance:`, margin, payY, { size: 8, color: COLORS.TEXT_MUTED })
                text(`R ${secondPaymentVal.toFixed(2)} (${secondPayStatus})`, margin + 25, payY, { size: 8, color: secondPayStatus === 'Approved' ? [16, 124, 65] : [220, 100, 0], style: 'bold' })
                payY += 5
            }
            ...
            if (data.client_signature) {
                try {
                    ...
                    const sigBoxY = payY + 4
                    
                    doc.setDrawColor(...COLORS.BORDER)
                    doc.setLineWidth(0.2)
                    doc.line(margin, sigBoxY, margin + 60, sigBoxY)
                    
                    doc.addImage(signatureImg, 'PNG', margin, sigBoxY + 2, 45, 12)
                    
                    text("CLIENT SIGNATURE", margin, sigBoxY + 18, { size: 7, color: COLORS.TEXT_MUTED })
                    text(`Digitally Signed: ${new Date(data.accepted_at || new Date()).toLocaleString()}`, margin, sigBoxY + 22, { size: 6, color: [0, 128, 0] })
                } catch (e) {
                    console.warn('Error rendering signature on first page:', e)
                }
            }
        }
        ...
        // ==========================================
        // 5. SITE PLAN PAGE (Quotations & Invoices with site plans)
        // ==========================================
        if ((docType === 'Quotation' || docType === 'Invoice') && data.site_plan_url) {
            try {
                doc.addPage()
                let planY = 20
                ...
                text(`Site Drawing - Doc #${data.id.substring(0, 8).toUpperCase()}`, margin, planY, { size: 9, color: COLORS.SECONDARY })
                planY += 10
...
/**
 * fetchImage — CORS-safe image loader for jsPDF
 *
 * Why we can't just use new Image() with crossOrigin="Anonymous":
 *   Supabase Storage serves assets from a separate domain. Even with the CORS
 *   header sent correctly, jsPDF reads the image data by drawing it to a canvas
 *   element — and the browser marks that canvas as "origin-tainted" if the image
 *   has already been decoded without CORS credentials (race condition in some
 *   browsers). This causes a security error when jsPDF calls canvas.toDataURL().
 *
 * The reliable fix is to fetch the bytes ourselves via the Fetch API (which
 * respects CORS headers properly), convert to a Blob, then read as a base64
 * data URL — jsPDF accepts this directly without touching canvas at all.
 */
const fetchImage = (url) => {
    return new Promise((resolve, reject) => {
        ...
        // Use fetch() with explicit CORS mode to load the raw bytes
        fetch(url, { mode: 'cors', credentials: 'omit' })
            ...
            .then(blob => {
                // Convert Blob → base64 data URL (jsPDF accepts this natively)
            ...
            .catch(fetchErr => {
                // Fallback: try the old Image() approach in case the URL is
                // same-origin (e.g. /logo.png served from the Next.js app itself)
                console.warn('fetch() failed, trying Image() fallback:', fetchErr)
                const img = new Image()
