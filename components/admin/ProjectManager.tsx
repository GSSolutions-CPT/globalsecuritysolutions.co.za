...
export function ProjectManager() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-bold text-brand-navy">Project Manager</h2>
                <p className="text-brand-steel">Add new completed installations to your gallery.</p>
            </div>
            ...
            <div className="bg-white p-8 rounded-2xl border border-brand-steel/40 shadow-sm">
                <form onSubmit={handleProjectSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-bold text-brand-slate mb-2">Project Title</label>
                            <input type="text" value={projTitle} onChange={e => setProjTitle(e.target.value)} className="w-full p-3 border border-brand-steel/40 rounded-xl focus:ring-2 focus:ring-brand-electric outline-none" placeholder="e.g. Luxury Estate Perimeter" required />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-brand-slate mb-2">Category</label>
                            <div className="relative">
                                <select value={projCategory} onChange={e => setProjCategory(e.target.value)} className="w-full p-3 border border-brand-steel/40 rounded-xl bg-brand-white focus:bg-white appearance-none pr-10">
                                ...
                                <Briefcase className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-steel pointer-events-none" />
                            </div>
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-brand-slate mb-2">Project Photo</label>
                        <div className="border-2 border-dashed border-brand-steel/60 rounded-xl p-8 text-center cursor-pointer hover:bg-brand-white transition-colors relative group">
                            ...
                            <label htmlFor="projImg" className="cursor-pointer w-full h-full flex flex-col items-center justify-center">
                                ...
                                <span className="text-brand-steel font-medium">{projImage ? <span className="text-green-600 font-bold">{projImage.name}</span> : 'Click to Upload Project Photo'}</span>
                            </label>
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-brand-slate mb-2">Description</label>
                        <textarea value={projDesc} onChange={e => setProjDesc(e.target.value)} className="w-full p-3 border border-brand-steel/40 rounded-xl h-32 focus:ring-2 focus:ring-brand-electric outline-none" placeholder="Describe the installation details..." required />
                    </div>
