...
import { Plus, Banknote, TrendingUp, TrendingDown, Receipt, Pencil, Trash2, Paperclip, FileText, Calendar, Wallet, PieChart, ArrowUpRight, ArrowDownRight, Loader2 } from 'lucide-react'
...
export default function FinancialsPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                type: formData.type as 'job' | 'general',
                amount: amountNum,
    ...
    const calculateMetrics = () => {
        ...
        const totalProfit = invoices
            .filter(inv => inv.status === 'Paid')
            .reduce((sum, inv) => sum + parseFloat(inv.profit_estimate?.toString() || '0'), 0)
    ...
    const prepareExpenseBreakdown = () => {
        ...
        expenses.forEach(exp => {
            const expType = exp.type as 'job' | 'general'
            breakdown[expType] += parseFloat(exp.amount?.toString() || '0')
        })
                                                    ...
                                                    <SelectContent>
                                                        {jobs.map((job) => (
                                                            <SelectItem key={job.id} value={job.id!}>
                                                                {job.clients?.name} - {job.scheduled_datetime ? new Date(job.scheduled_datetime).toLocaleDateString() : 'Unscheduled'}
                                                            </SelectItem>
                                                        ))}
                                                    </SelectContent>
                                                </Select>
