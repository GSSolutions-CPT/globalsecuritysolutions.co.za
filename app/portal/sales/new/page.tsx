...
import { Client, Product, SitePlan } from '@/types/crm'
...
function CreateSaleContent() {
    ...
    const [mode, setMode] = useState<'quotation' | 'invoice'>(defaultType === 'invoice' ? 'invoice' : 'quotation')
    ...
    const [sitePlanPreview, setSitePlanPreview] = useState<string | null>(null)
    ...
    const fetchSitePlan = useCallback(async (quotationId: string) => {
        try {
            const { data, error } = await supabase
                .from('site_plans')
                .select('*')
    ...
    const fetchEditData = useCallback(async (id: string, type: string) => {
        try {
            setIsLoading(true)
    ...
    useEffect(() => {
        ...
        if (editParam) {
            setEditId(editParam)
            if (typeParam) setMode(typeParam as 'quotation' | 'invoice')
            fetchEditData(editParam, typeParam || 'quotation')
            if (!typeParam || typeParam === 'quotation') {
                fetchSitePlan(editParam)
            }
        }
    }, [fetchClients, fetchProducts, fetchEditData, fetchSitePlan, searchParams])

    useEffect(() => {
        if (mode === 'quotation' && !formData.valid_until && settings) {
            const days = parseInt(settings.defaultQuoteValidityDays as string) || 14
            const date = new Date()
    ...
    const saveDraftForSitePlan = async () => {
        ...
        try {
            ...
            const { data: saleData, error: saleError } = await supabase
                ...
                .insert([payload])
                .select()
            if (saleError || !saleData) throw saleError || new Error('Failed to create draft')
